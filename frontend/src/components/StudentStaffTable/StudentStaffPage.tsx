...
export default function StudentStaffPage({ type, state }: StudentStaffPageProps) {
  ...
  const toastTriggerRef = React.useRef(0);

  const getNextToastTrigger = () => {
    toastTriggerRef.current += 1;
    return toastTriggerRef.current;
  };
  ...
  const data = useMemo<Student[] | User[]>(() => {
    ...
    if (type === "student") {
      ...
      return asc ? base : [...base].reverse();
    } else {
      let base = (root as User[]).filter((s) => s.archived === showArchived);
  ...
  }, [root, activeView, searchFilter, selected, asc, sortBy, type]);
  ...
  const undoArchive = (ids: string[]) => {
    ...
    if (type === "student") {
      ...
      console.log("undoing archive for staff");
    }
  };
  /**
  ...
  const undoUnarchive = (ids: string[]) => {
    ...
    if (type === "student") {
      ...
      console.log("undoing unarchive");
    }
  };

  /**
   * Bulk unarchives data
   */
  const unarchive = () => {
    const ids = Array.from(selected);
    if (ids.length === 0) {
  ...
  const renderHeader = () => {
    let archiveLabel = "";
    if (type === "student") {
      archiveLabel = activeView ? "Archive Students" : "Unarchive Students";
    } else {
      archiveLabel = activeView ? "Archive Staff" : "Unarchive Staff";
    }
    const archiveFxn = activeView ? archive : unarchive;
    return (
      <div className={`${styles.headerBar} ${styles.top}`}>
        <div className={styles.headerContainer}>
          <h1>{type === "student" ? "Students" : "Staff"}</h1>
              ...
              {state === "admin" && (
                <>
                  ...
                  >
                    ...
                    {type === "student" ? "Delete Students" : "Delete Staff"}
                  </button>
                </>
              )}
            </>
      ...
      <div className={`${styles.headerBar}`}>
        ...
        <div className={styles.headerContainer}>
          ...
          <div className={styles.searchWrapper}>
            ...
            <input
              type="text"
              placeholder={type === "student" ? "Search Students" : "Search Staff"}
            ...
            type === "student" ? (
              <StudentForm
                ...
                onSubmit={(new_stud: Student) => {
                  ...
                  getAllStudents()
                    .then((result) => {
                      if (result.success) setRoot(result.data);
